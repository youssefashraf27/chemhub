...
-- الجداول الخاصة بالاختبارات: لو كانت غير موجودة يتم إنشاؤها أولاً.
-- هذا يمنع الخطأ 42P01: relation "public.quizzes" does not exist.
...
create policy "quizzes_student_select" on public.quizzes for select to authenticated
...
create policy "questions_student_select" on public.questions for select to authenticated
using (exists (select 1 from public.quizzes q where q.id = quiz_id and q.is_active = true));
...
create policy "quiz_results_student_select" on public.quiz_results for select to authenticated
using (user_id = auth.uid());

drop policy if exists "quiz_results_student_insert" on public.quiz_results;
create policy "quiz_results_student_insert" on public.quiz_results for insert to authenticated
