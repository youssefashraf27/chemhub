...
function renderServices(list){
  ...
  box.innerHTML = list.map(s => {
    ...
    */
    ...
    /*
      بنك الاختبارات:
      لا نعتمد على قيمة link الموجودة في قاعدة البيانات،
      لأن الخدمة القديمة قد تكون محفوظة برابط # أو برابط خاطئ.
    */
    const serviceTitle = String(s.title || '').trim();
    if(!isAI && (
      serviceTitle.includes('بنك الاختبارات') ||
      serviceTitle.includes('الاختبارات') ||
      serviceTitle.toLowerCase().includes('quiz')
    )){
      href = 'quizzes.html';
    }
