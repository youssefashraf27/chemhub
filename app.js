...
setupAuthUI().catch(() => renderAuthUI(null));
...
function renderYear(year){
  ...
  Object.entries(data).forEach(([semester, subjects])=>{
    ...
    if(!subjects.length){
      html += `<div class="card empty">لم تتم إضافة مواد هذه الفرقة بعد.</div>`;
    }else{
      html += `<div class="subject-grid">`;

      subjects.forEach(([name,teacher,grade])=>{
        html += `
          <a class="subject" href="subject.html?subject=${encodeURIComponent(name)}">
            <div>
              <strong>${name}</strong>
              <small>${teacher}</small>
            </div>
...
$$(".year-card").forEach(card=>{
  card.addEventListener("click",()=>renderYear(card.dataset.year));
});
...
function renderCourses(){
  ...
  if(!courses.length){
    list.innerHTML = '<p class="empty">لم تتم إضافة مواد بعد.</p>';
  }else{
    list.innerHTML = courses.map((c,i)=>`
      <div class="course-row">
        <span>${c.name || "مادة"}</span>
        <span>${c.grade}/100</span>
  ...
  courses.forEach(c=>{
    points += gradePoint(Number(c.grade))*Number(c.hours);
...
$("#addCourse").addEventListener("click",()=>{
  ...
  if(!grade || grade<0 || grade>100 || !hours){
    alert("أدخل درجة صحيحة وعدد الساعات.");
    return;
  }
