...
create index if not exists ch_chat_messages_grade_created_idx
on public.ch_chat_messages(grade, created_at desc);
...
begin
  ...
  if p.id is null then
    raise exception 'لا يوجد ملف مستخدم لهذا الحساب';
  end if;

  if p.grade is null or trim(p.grade) = '' then
    raise exception 'يجب تحديد الفرقة أولاً';
  end if;
...
create policy ch_chat_select
on public.ch_chat_messages for select
to authenticated
using (
  public.is_admin()
...
create policy ch_chat_insert
on public.ch_chat_messages for insert
to authenticated
with check (
  sender_id = auth.uid()
...
create policy ch_chat_delete
on public.ch_chat_messages for delete
to authenticated
using (sender_id = auth.uid() or public.is_admin());

drop policy if exists ch_chat_update_admin on public.ch_chat_messages;
create policy ch_chat_update_admin
on public.ch_chat_messages for update
to authenticated
...
create policy chemistryhub_chat_files_select
...
using (
  ...
  and (
    ...
    or split_part(name,'/',1) = (
      select p.grade from public.profiles p where p.id=auth.uid()
    )
  )
);

drop policy if exists chemistryhub_chat_files_insert on storage.objects;
create policy chemistryhub_chat_files_insert
...
create policy chemistryhub_chat_files_delete
...
begin
  if not exists (
    ...
    where pubname='supabase_realtime'
      ...
      and tablename='ch_chat_messages'
  ) then
