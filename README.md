...
من `admin.html` يمكن للمدير:
- إضافة/تعديل/حذف المواد والفرقة والفصل والمدرس.
- رفع وحذف ملفات PDF داخل Supabase Storage الخاص (Private Bucket).
- إنشاء وتعديل وحذف الاختبارات والأسئلة والإجابات الصحيحة.
- إدارة المستخدمين وترقية/خفض الصلاحيات.
- تعديل النصوص الأساسية وروابط التواصل وإشعار الصفحة الرئيسية.

الموقع لم يعد يعتمد على تعديل `data.js` لإدارة المحتوى الأساسي؛ البيانات الجديدة تأتي من Supabase. `data.js` موجود فقط كملف قديم/احتياطي.
